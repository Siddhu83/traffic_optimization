...
if 'history_df' not in st.session_state:
    st.session_state.history_df = pd.DataFrame(columns=['Time', 'N', 'S', 'E', 'W'])
if 'co2_saved' not in st.session_state:
    st.session_state.co2_saved = 0.0
if 'last_emergency_state' not in st.session_state:
    st.session_state.last_emergency_state = False
...
st.session_state.co2_saved += 2.3
...
if len(st.session_state.history_df) > 60:
    st.session_state.history_df = st.session_state.history_df.tail(60)
...
if emergency_detected:
    ...
    if not st.session_state.last_emergency_state:
...
# The Impact Meter
st.metric(label="🌱 Live CO2 Impact", value=f"{st.session_state.co2_saved:.1f} g Saved", delta="2.3 g/s", delta_color="normal")
...
# Plot lines
df_plot = st.session_state.history_df.set_index('Time')
st.line_chart(df_plot, use_container_width=True)

st.subheader("Agent Reasoning Box")
if agent_logs:
    log_text = ""
    for log in agent_logs[-3:]: # Show last 3 logs
        log_text += f"**[{log.get('agent', 'System')}]** 🤔 {log.get('message', '')}\n\n"
    st.info(log_text)
else:
    st.info("No active agent logs.")

st.session_state.last_emergency_state = emergency_detected
time.sleep(1)
st.rerun()
